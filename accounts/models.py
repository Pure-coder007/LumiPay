...
class TransactionHistory(models.Model):
    ...
    transaction_id = models.CharField(max_length=12, unique=True, default=random_transaction_id)
    session_id = models.CharField(max_length=12, unique=True, default=random_session_id)
    sender = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='sent_transactions', on_delete=models.SET_NULL)
    receiver = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='received_transactions', on_delete=models.SET_NULL)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
